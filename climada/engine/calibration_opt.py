...
def calib_instance(
    ...
    impacts = ImpactCalc(exposures=exposure, impfset=ifs, hazard=hazard).impact(
...
def init_impact_data(
    ...
    if impact_data_source == "emdat":
        if yearly_impact:
            em_data = emdat_impact_yearlysum(
                source_file,
                countries=region_ids,
                hazard=hazard_type,
            ...
            raise ValueError(
                "init_impact_data not yet implemented for yearly_impact = False."
            )
...
def calib_cost_calc(df_out, cost_function):
    ...
    if cost_function == "R2":
        cost = np.sum(
            (
                pd.to_numeric(df_out["impact_scaled"])
                - pd.to_numeric(df_out["impact_CLIMADA"])
            )
            ** 2
        )
        ...
        impact2[impact2 <= 0] = 1
        cost = np.sum((np.log(impact1) - np.log(impact2)) ** 2)
    else:
...
def calib_optimize(
    ...
    if res.success:
        ...
        LOGGER.info(
            "Opimization did not finish successfully. Check you input"
