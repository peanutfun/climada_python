...
def assign_hazard_to_emdat(
    ...
    if certainty_level not in certainty_levels:
        raise ValueError(
            "Invalid certainty level. Expected one of: %s" % certainty_levels
        )

    # prepare hazard set
    print("Start preparing hazard set")
    hit_countries = hit_country_per_hazard(
        intensity_path_haz, names_path_haz, reg_id_path_haz, date_path_haz
    )
    # prepare damage set
    ...
    lookup = create_lookup(
        emdat_data, start_time, end_time, disaster_subtype="Tropical cyclone"
    )
    ...
    if certainty_level == "high":
        lookup = assign_track_to_em(
            lookup=lookup,
            possible_tracks_1=hit10_match,
            possible_tracks_2=hit50_match,
            level=1,
        )
        lookup = assign_track_to_em(
            lookup=lookup,
            possible_tracks_1=hit15_match,
            possible_tracks_2=hit50_match,
            level=2,
        )
        lookup = assign_track_to_em(
            lookup=lookup,
            possible_tracks_1=hit25_match,
            possible_tracks_2=hit50_match,
            level=3,
        )
        lookup = assign_track_to_em(
    ...
    # assign all tracks
        ...
        lookup = assign_track_to_em(
            lookup=lookup,
            possible_tracks_1=hit5_match,
            possible_tracks_2=hit5_match,
            level=12,
        )
    if not keep_checks:
        lookup = lookup.drop(
            ["Date_start_EM_ordinal", "possible_track", "possible_track_all"], axis=1
        )
    lookup.groupby("allocation_level").count()
    print(
        "(%d/%s) tracks allocated"
...
def emdat_possible_hit(lookup, hit_countries, delta_t):
    ...
    for i in range(0, len(lookup.EM_ID.values)):
        ...
        for j in range(0, len(country_tracks.Date_start.values)):
            if (
                lookup.Date_start_EM_ordinal.values[i]
    ...
    for i, _ in enumerate(possible_tracks_1):
        if np.isnan(lookup.allocation_level.values[i]):
            ...
            for j in range(0, number_emdat_id):
                ...
                if (
                    len(possible_tracks_1[i][j]) == len(possible_tracks_2[i][j]) == 1
                    and possible_tracks_1[i][j] != []
                ):
                    # check that all tracks are the same
                    if all(
                        possible_tracks_1[i][0] == possible_tracks_1[i][k]
                        for k in range(0, len(possible_tracks_1[i]))
                    ):
                        # check that track ID has not been assigned to that country already
                        ctry_lookup = lookup[
                            lookup["hit_country"] == lookup.hit_country.values[i]
                        ]
                        if (
                            possible_tracks_1[i][0][0]
                            not in ctry_lookup.ibtracsID.values
                        ):
...
def check_assigned_track(lookup, checkset):
    ...
    check = pd.merge(
        checkset,
        lookup[["hit_country", "EM_ID", "ibtracsID"]],
    ...
    print(
        "%.1f%% tracks assigned correctly, %.1f%% wrongly, %.1f%% not assigned"
        % (
...
def clean_emdat_df(
    emdat_file, countries=None, hazard=None, year_range=None, target_version=None
):
    """
    ...
    for vers in sorted(VARNAMES_EMDAT.keys()):
        if len(df_emdat.columns) >= len(VARNAMES_EMDAT[vers]) and all(
            item in list(df_emdat.columns) for item in VARNAMES_EMDAT[vers].values()
        ):
            version = vers
    if not version:
        raise ValueError(
            "the given emdat_file contains unexpected columns and cannot be"
    ...
    df_data = pd.DataFrame(
        index=df_emdat.index.values, columns=VARNAMES_EMDAT[target_version].values()
    )
    ...
    if year_range:
        for idx in df_data.index:
            if np.isnan(df_data.loc[0, "Year"]):
                df_data.loc[0, "Year"] = df_data.loc[
                    0, VARNAMES_EMDAT[target_version]["Start Year"]
                ]
        df_data = df_data[
            (df_data["Year"] >= min(year_range)) & (df_data["Year"] <= max(year_range))
        ]
    ...
    if hazard and isinstance(hazard, list):
        ...
        for idx, haz in enumerate(hazard):
            ...
            if (
                haz
                in df_data[VARNAMES_EMDAT[target_version]["Disaster Subtype"]].unique()
            ):
        ...
        df_data = df_data[
            (
                df_data[VARNAMES_EMDAT[target_version]["Disaster Type"]].isin(
                    disaster_types
                )
            )
            | (
                df_data[VARNAMES_EMDAT[target_version]["Disaster Subtype"]].isin(
                    disaster_subtypes
                )
            )
        ]
...
def emdat_impact_event(
    ...
    for country in df_data.ISO.unique():
        try:
            df_data.loc[df_data.ISO == country, "region_id"] = u_coord.country_to_iso(
                country, "numeric"
            )
        except LookupError:
...
def emdat_to_impact(
    ...
    if isinstance(countries, str):
        ...
        countries = emdat_countries_by_hazard(
            emdat_file_csv, year_range=year_range, hazard=hazard_type_emdat
        )[0]

    if em_data.empty:
        return impact_instance, countries
    impact_instance.event_id = np.array(em_data.index, int)
    impact_instance.event_name = list(em_data[VARNAMES_EMDAT[version]["Dis No"]])

    date_list = list()
    for year in list(em_data["Year"]):
        date_list.append(datetime.toordinal(datetime.strptime(str(year), "%Y")))
    if (
        "Start Year" in em_data.columns
        and "Start Month" in em_data.columns
        and "Start Day" in em_data.columns
    ):
        idx = 0
        for year, month, day in zip(
            em_data["Start Year"], em_data["Start Month"], em_data["Start Day"]
        ):
            if np.isnan(year):
                idx += 1
                continue
            ...
            date_list[idx] = datetime.toordinal(
                datetime.strptime("%02i/%02i/%04i" % (day, month, year), "%d/%m/%Y")
            )
            idx += 1
    ...
    impact_instance.aai_agg = np.nansum(
        impact_instance.at_event * impact_instance.frequency
    )
    ...
    for idx, cntry in enumerate(countries):
        ...
        if not reference_year:
            ...
            impact_instance.eai_exp[idx] = sum(
                np.array(df_tmp["impact_scaled"]) * impact_instance.frequency[0]
            )
