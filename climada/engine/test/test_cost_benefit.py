...
class TestSteps(unittest.TestCase):
    ...
    def test_calc_impact_measures_pass(self):
        ...
        cost_ben._calc_impact_measures(
            hazard,
            entity.exposures,
            entity.measures,
            entity.impact_funcs,
        ...
        self.assertAlmostEqual(
            cost_ben.imp_meas_future["no measure"]["risk"],
            6.51220115756442e09,
            places=3,
        )
        ...
        self.assertTrue(
            np.allclose(
                new_efc.impact, cost_ben.imp_meas_future["no measure"]["efc"].impact
            )
        )
        self.assertEqual(
        ...
        self.assertAlmostEqual(
            ...
            1.3117683608515418e09,
            places=3,
        )
        self.assertEqual(cost_ben.imp_meas_future["Mangroves"]["cost"][1], 1)
        self.assertAlmostEqual(
            cost_ben.imp_meas_future["Mangroves"]["risk"],
            4.850407096284983e09,
            places=3,
        )
        ...
        self.assertTrue(
            np.allclose(
                new_efc.impact, cost_ben.imp_meas_future["Mangroves"]["efc"].impact
            )
        )
        self.assertEqual(
        ...
        self.assertEqual(
            cost_ben.imp_meas_future["Beach nourishment"]["impact"]
            .at_event.nonzero()[0]
            .size,
            702,
        )
        self.assertEqual(
            cost_ben.imp_meas_future["Beach nourishment"]["impact"].at_event[1110], 0.0
        )
        self.assertAlmostEqual(
            cost_ben.imp_meas_future["Beach nourishment"]["impact"].eai_exp[5],
            1.1133679079730146e08,
            places=3,
        )
        self.assertAlmostEqual(
        ...
        self.assertAlmostEqual(
            ...
            8.878779433630093e09,
            places=3,
        )
        self.assertEqual(cost_ben.imp_meas_future["Seawall"]["cost"][1], 1)
        self.assertAlmostEqual(
            cost_ben.imp_meas_future["Seawall"]["risk"], 4.736400526119911e09, places=3
        )
        ...
        self.assertTrue(
            np.allclose(
                new_efc.impact, cost_ben.imp_meas_future["Seawall"]["efc"].impact
            )
        )
        self.assertEqual(
            cost_ben.imp_meas_future["Seawall"]["impact"].at_event.nonzero()[0].size, 73
        )
        self.assertEqual(
        ...
        self.assertTrue(
            np.allclose(
                new_efc.impact, cost_ben.imp_meas_future["Building code"]["efc"].impact
            )
        )
        self.assertEqual(
            cost_ben.imp_meas_future["Building code"]["impact"]
            .at_event.nonzero()[0]
            .size,
            841,
        )
        self.assertEqual(
            cost_ben.imp_meas_future["Building code"]["impact"].at_event[122], 0.0
        )
        self.assertAlmostEqual(
            cost_ben.imp_meas_future["Building code"]["impact"].eai_exp[11],
            7.757060129393841e07,
            places=3,
        )
        self.assertAlmostEqual(
    ...
    def test_cb_one_meas_pres_pass(self):
        ...
        meas_name = "Mangroves"
        meas_val = dict()
        meas_val["cost"] = (1.3117683608515418e09, 1)
        meas_val["risk"] = 4.826231151473135e10
        meas_val["efc"] = None
        meas_val["risk_transf"] = 0

        imp_meas_present = dict()
        imp_meas_present["no measure"] = dict()
        imp_meas_present["no measure"]["risk"] = 6.51220115756442e09
        imp_meas_present["Mangroves"] = dict()
        imp_meas_present["Mangroves"]["risk"] = 4.850407096284983e09
        imp_meas_present["Mangroves"]["risk_transf"] = 0

        imp_meas_future = dict()
        imp_meas_future["no measure"] = dict()
        imp_meas_future["no measure"]["risk"] = 5.9506659786664024e10

        cb = CostBenefit(
            present_year=2018,
            future_year=2040,
            imp_meas_present=imp_meas_present,
    ...
    def test_cb_one_meas_fut_pass(self):
        ...
        meas_val = dict()
        meas_val["cost"] = (1.3117683608515418e09, 1)
        meas_val["risk"] = 4.850407096284983e09
        meas_val["efc"] = None
        meas_val["risk_transf"] = 0

        imp_meas_future = dict()
        imp_meas_future["no measure"] = dict()
        imp_meas_future["no measure"]["risk"] = 6.51220115756442e09

        cb = CostBenefit(
            present_year=2018, future_year=2040, imp_meas_future=imp_meas_future
        )
        ...
        cb._cost_ben_one(meas_name, meas_val, disc_rates, time_dep)
        self.assertAlmostEqual(cb.benefit[meas_name], 3.100583368954022e10, places=3)
    ...
    def test_calc_cb_no_change_pass(self):
        ...
        self.assertAlmostEqual(
            cost_ben.cost_ben_ratio["Mangroves"], 0.04230714690616641
        )
        self.assertAlmostEqual(
            cost_ben.cost_ben_ratio["Beach nourishment"], 0.06998836431681373
        )
        self.assertAlmostEqual(cost_ben.cost_ben_ratio["Seawall"], 0.2679741183248266)
        self.assertAlmostEqual(
            cost_ben.cost_ben_ratio["Building code"], 0.30286828677985717
        )

        self.assertAlmostEqual(
            cost_ben.benefit["Mangroves"], 3.100583368954022e10, places=3
        )
        self.assertAlmostEqual(
            cost_ben.benefit["Beach nourishment"], 2.468981832719974e10, places=3
        )
        self.assertAlmostEqual(
            cost_ben.benefit["Seawall"], 3.3132973770502796e10, places=3
        )
        self.assertAlmostEqual(
            cost_ben.benefit["Building code"], 3.0376240767284798e10, places=3
        )

        self.assertAlmostEqual(
            cost_ben.tot_climate_risk, 1.2150496306913972e11, places=3
        )
    ...
    def test_calc_cb_change_pass(self):
        ...
        self.assertEqual(cost_ben.future_year, 2040)
        self.assertAlmostEqual(
            cost_ben.tot_climate_risk, 5.768659152882021e11, places=3
        )

        self.assertAlmostEqual(
            cost_ben.imp_meas_present["no measure"]["risk"],
            6.51220115756442e09,
            places=3,
        )
        self.assertAlmostEqual(
            cost_ben.imp_meas_present["Mangroves"]["risk"],
            4.850407096284983e09,
            places=3,
        )
        self.assertAlmostEqual(
        ...
        self.assertAlmostEqual(
            cost_ben.imp_meas_present["Seawall"]["risk"], 4.736400526119911e09, places=3
        )
        self.assertAlmostEqual(
        ...
        self.assertAlmostEqual(
            cost_ben.imp_meas_future["no measure"]["risk"],
            5.9506659786664024e10,
            places=3,
        )
        self.assertAlmostEqual(
            cost_ben.imp_meas_future["Mangroves"]["risk"],
            4.826231151473135e10,
            places=3,
        )
        self.assertAlmostEqual(
            cost_ben.imp_meas_future["Beach nourishment"]["risk"],
            5.0647250923231674e10,
            places=3,
        )
        self.assertAlmostEqual(
            cost_ben.imp_meas_future["Seawall"]["risk"], 21089567135.7345, places=3
        )
        self.assertAlmostEqual(
            cost_ben.imp_meas_future["Building code"]["risk"],
            4.462999483999791e10,
            places=3,
        )

        self.assertAlmostEqual(
            cost_ben.benefit["Mangroves"], 113345027690.81276, places=2
        )
        self.assertAlmostEqual(
            cost_ben.benefit["Beach nourishment"], 89444869971.53653, places=2
        )
        self.assertAlmostEqual(cost_ben.benefit["Seawall"], 347977469896.1333, places=2)
        self.assertAlmostEqual(
            cost_ben.benefit["Building code"], 144216478822.05154, places=2
        )

        self.assertAlmostEqual(
            cost_ben.cost_ben_ratio["Mangroves"], 0.011573232523528404
        )
        self.assertAlmostEqual(
            cost_ben.cost_ben_ratio["Beach nourishment"], 0.01931916274851638
        )
        self.assertAlmostEqual(cost_ben.cost_ben_ratio["Seawall"], 0.025515385913577368)
        self.assertAlmostEqual(
            cost_ben.cost_ben_ratio["Building code"], 0.06379298728650741
        )
    ...
    def test_npv_unaverted_no_pres_pass(self):
        ...
        res = cb._npv_unaverted_impact(
            risk_future, disc_rates, time_dep, risk_present=None
        )

        self.assertEqual(
            res,
            disc_rates.net_present_value(
                cb.present_year, cb.future_year, time_dep * risk_future
            ),
        )
    ...
    def test_npv_unaverted_pres_pass(self):
        ...
        self.assertEqual(
            res,
            disc_rates.net_present_value(
                cb.present_year, cb.future_year, tot_climate_risk
            ),
        )
    ...
    def test_combine_fut_pass(self):
        ...
        new_cb = cost_ben.combine_measures(
            ["Mangroves", "Seawall"],
            new_name,
            new_color,
            entity.disc_rates,
        ...
        new_imp = np.maximum(
            cost_ben.imp_meas_future["no measure"]["impact"].at_event - new_imp, 0
        )

        self.assertTrue(
            np.allclose(new_cb.imp_meas_present[new_name]["impact"].at_event, new_imp)
        )
        self.assertAlmostEqual(
            new_cb.imp_meas_present[new_name]["risk"],
            np.sum(
                new_imp * cost_ben.imp_meas_present["no measure"]["impact"].frequency
            ),
            5,
        )
        self.assertAlmostEqual(
        ...
        self.assertTrue(
            np.allclose(new_cb.imp_meas_future[new_name]["impact"].at_event, new_imp)
        )
        self.assertAlmostEqual(
            new_cb.imp_meas_future[new_name]["risk"],
            np.sum(
                new_imp * cost_ben.imp_meas_future["no measure"]["impact"].frequency
            ),
            5,
        )
        self.assertAlmostEqual(
    ...
    def test_apply_transf_current_pass(self):
        ...
        self.assertEqual(len(new_cb.imp_meas_present), 0)
        self.assertTrue(
            np.allclose(new_cb.imp_meas_future[tr_name]["impact"].at_event, new_imp)
        )
        self.assertAlmostEqual(
        ...
        self.assertAlmostEqual(
            new_cb.imp_meas_future[tr_name]["risk_transf"], risk_transfer
        )
    ...
    def test_apply_transf_future_pass(self):
        ...
        self.assertTrue(
            np.allclose(new_cb.imp_meas_present[tr_name]["impact"].at_event, new_imp)
        )
        self.assertAlmostEqual(
            new_cb.imp_meas_future[tr_name]["risk"],
            np.sum(
...
class TestRiskFuncs(unittest.TestCase):
    ...
    def test_risk_aai_agg_pass(self):
        ...
        self.assertAlmostEqual(6.512201157564421e09, risk, 5)
        self.assertTrue(np.isclose(6.512201157564421e09, risk))
