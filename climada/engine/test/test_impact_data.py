...
class TestEmdatProcessing(unittest.TestCase):
    def test_emdat_impact_event_2018(self):
        ...
        self.assertEqual(df["Total damage ('000 US$)"][0], df["impact"][0] * 1e-3)
        self.assertEqual(df["impact_scaled"][1], df["impact"][1])
        ...
        self.assertAlmostEqual(df["impact_scaled"][0] * 1e-7, 192.7868, places=0)
    ...
    def test_emdat_impact_event_2020(self):
        ...
        self.assertEqual(df["Total Damages ('000 US$)"][0], df["impact"][0] * 1e-3)
        self.assertNotEqual(df["impact_scaled"][1], df["impact"][1])
        ...
        self.assertAlmostEqual(df["impact_scaled"][0] * 1e-9, 1.012, places=0)
        self.assertIn("USA", list(df["ISO"]))
        self.assertIn("Drought", list(df["Disaster Type"]))
    ...
    def test_emdat_countries_by_hazard_2020_pass(self):
        ...
        iso3_codes, country_names = im_d.emdat_countries_by_hazard(
            EMDAT_2020_CSV_DEMO, hazard="TC", year_range=(2000, 2019)
        )
...
class TestEmdatToImpact(unittest.TestCase):
    ...
    def test_emdat_to_impact_all_countries_pass(self):
        ...
        # file 1: version 2020
        _impact_emdat2020, countries2020 = im_d.emdat_to_impact(
            EMDAT_2020_CSV_DEMO, "TC"
        )
        # file 2: version 2018
        ...
        self.assertAlmostEqual(
            555861710000 * 1e-5, np.sum(impact_emdat.at_event) * 1e-5, places=0
        )
        self.assertAlmostEqual(
            0.0208333333333, np.unique(impact_emdat.frequency)[0], places=7
        )
    ...
    def test_emdat_to_impact_fakedata(self):
        ...
        impact_emdat, countries = im_d.emdat_to_impact(
            EMDAT_TEST_CSV_FAKE, "FL", hazard_type_emdat="Flood"
        )
    ...
    def test_emdat_to_impact_2020format(self):
        ...
        df1 = im_d.clean_emdat_df(
            EMDAT_2020_CSV_DEMO, hazard="TC", countries="PHL", year_range=(2013, 2013)
        )
        df2 = im_d.emdat_impact_event(
            EMDAT_2020_CSV_DEMO,
            countries="PHL",
            hazard="TC",
        ...
        self.assertAlmostEqual(17944571.0, impact_emdat.aai_agg, places=0)
