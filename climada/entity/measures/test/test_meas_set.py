...
class TestChecker(unittest.TestCase):
    ...
    def test_check_wronginten_fail(self):
        ...
        with self.assertRaises(ValueError) as cm:
            meas.check()
        self.assertIn(
            "Invalid Measure.hazard_inten_imp size: 2 != 3.", str(cm.exception)
        )
    ...
    def test_check_wrongColor_fail(self):
        ...
        with self.assertRaises(ValueError) as cm:
            meas.check()
        self.assertIn(
            "Invalid Measure.color_rgb size: 2 not in [3, 4].", str(cm.exception)
        )
    ...
    def test_def_color(self):
        ...
        meas.check()
        self.assertTrue(
            np.array_equal(meas.get_measure("FL", "LaLa").color_rgb, np.ones(4))
        )
        self.assertTrue(
            np.allclose(
                meas.get_measure("FL", "LoLo").color_rgb, np.array([0.0, 0.0, 0.0, 1.0])
            )
        )
...
class TestReaderExcel(unittest.TestCase):
    ...
    def test_template_file_pass(self):
        ...
        self.assertEqual(
            meas.get_measure(name="enforce building code")[0].imp_fun_map, "1to3"
        )
        ...
        self.assertEqual(act_buil.haz_type, "TC")
        self.assertTrue(
            np.array_equal(act_buil.color_rgb, np.array([0.90, 0.72, 0.72]))
        )
