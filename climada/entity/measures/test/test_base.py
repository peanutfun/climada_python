...
class TestApply(unittest.TestCase):
    ...
    def test_change_imp_func_pass(self):
        ...
        self.assertTrue(
            np.array_equal(
                new_imp.intensity,
                np.array([4.0, 24.0, 34.0, 44.0, 54.0, 64.0, 74.0, 84.0, 104.0]),
            )
        )
        self.assertTrue(
            np.array_equal(
    ...
    def test_change_exposures_impf_pass(self):
        ...
        self.assertTrue(
            np.array_equal(new_exp.gdf.latitude.values, exp.gdf.latitude.values)
        )
        self.assertTrue(
            np.array_equal(new_exp.gdf.longitude.values, exp.gdf.longitude.values)
        )
        self.assertTrue(
            np.array_equal(
                exp.gdf[INDICATOR_IMPF + "TC"].values, np.ones(new_exp.gdf.shape[0])
            )
        )
        self.assertTrue(
            np.array_equal(
                new_exp.gdf[INDICATOR_IMPF + "TC"].values,
    ...
    def test_change_all_exposures_pass(self):
        ...
        self.assertTrue(
            np.array_equal(new_exp.gdf.latitude.values, ref_exp.gdf.latitude.values)
        )
        self.assertTrue(
            np.array_equal(new_exp.gdf.longitude.values, ref_exp.gdf.longitude.values)
        )
    ...
    def test_not_filter_exposures_pass(self):
        ...
        res_exp, res_ifs, res_haz = meas._filter_exposures(
            exp, imp_set, haz, new_exp, new_impfs, new_haz
        )
    ...
    def test_filter_exposures_pass(self):
        ...
        self.assertTrue(
            np.array_equal(res_exp.gdf.region_id.values[1:25], np.ones(24) * 3)
        )
        self.assertTrue(np.array_equal(res_exp.gdf.region_id.values[25:], np.ones(25)))

        # changed exposures
        self.assertTrue(
            np.array_equal(res_exp.gdf.value.values[:25], new_exp.gdf.value.values[:25])
        )
        self.assertTrue(
            np.all(
                np.not_equal(res_exp.gdf.value.values[:25], exp.gdf.value.values[:25])
            )
        )
        self.assertTrue(
            np.all(
                np.not_equal(
                    res_exp.gdf.impf_TC.values[:25], new_exp.gdf.impf_TC.values[:25]
                )
            )
        )
        self.assertTrue(
            np.array_equal(
                res_exp.gdf.latitude.values[:25], new_exp.gdf.latitude.values[:25]
            )
        )
        self.assertTrue(
            np.array_equal(
                res_exp.gdf.longitude.values[:25], new_exp.gdf.longitude.values[:25]
            )
        )

        # unchanged exposures
        self.assertTrue(
            np.array_equal(res_exp.gdf.value.values[25:], exp.gdf.value.values[25:])
        )
        self.assertTrue(
            np.all(
                np.not_equal(
                    res_exp.gdf.value.values[25:], new_exp.gdf.value.values[25:]
                )
            )
        )
        self.assertTrue(
            np.array_equal(res_exp.gdf.impf_TC.values[25:], exp.gdf.impf_TC.values[25:])
        )
        self.assertTrue(
            np.array_equal(
                res_exp.gdf.latitude.values[25:], exp.gdf.latitude.values[25:]
            )
        )
        self.assertTrue(
            np.array_equal(
                res_exp.gdf.longitude.values[25:], exp.gdf.longitude.values[25:]
            )
        )
        ...
        self.assertEqual(
            res_ifs.get_func()[meas.haz_type][1].id,
            imp_set.get_func()[meas.haz_type][1].id,
        )
        self.assertTrue(
            np.array_equal(
                res_ifs.get_func()[meas.haz_type][1].intensity,
                imp_set.get_func()[meas.haz_type][1].intensity,
            )
        )
        self.assertEqual(
            res_ifs.get_func()[meas.haz_type][3].id,
            imp_set.get_func()[meas.haz_type][3].id,
        )
        self.assertTrue(
            np.array_equal(
                res_ifs.get_func()[meas.haz_type][3].intensity,
        ...
        self.assertTrue(
            np.array_equal(
                res_haz.intensity[:, :36].toarray(), haz.intensity[:, :36].toarray()
            )
        )
        self.assertTrue(
            np.array_equal(
                res_haz.intensity[:, 37:46].toarray(), haz.intensity[:, 37:46].toarray()
            )
        )
        self.assertTrue(
            np.array_equal(
                res_haz.intensity[:, 47:].toarray(), haz.intensity[:, 47:].toarray()
            )
        )
        ...
        self.assertTrue(
            np.array_equal(
                res_haz.intensity[[36, 46]].toarray(),
    ...
    def test_apply_ref_pass(self):
        ...
        new_exp, new_ifs, new_haz = entity.measures.get_measure(
            "TC", "Mangroves"
        ).apply(entity.exposures, entity.impact_funcs, hazard)
    ...
    def test_calc_impact_pass(self):
        ...
        self.assertAlmostEqual(imp.aai_agg, 4.850407096284983e09, delta=1)
        self.assertAlmostEqual(imp.at_event[0], 0)
        self.assertAlmostEqual(imp.at_event[12], 1.470194187501225e07)
        self.assertAlmostEqual(imp.at_event[41], 4.7226357936631286e08)
        self.assertAlmostEqual(imp.at_event[11890], 1.742110428135755e07)
        self.assertTrue(
            np.array_equal(imp.coord_exp[:, 0], entity.exposures.gdf.latitude)
        )
        self.assertTrue(
            np.array_equal(imp.coord_exp[:, 1], entity.exposures.gdf.longitude)
        )
        self.assertAlmostEqual(imp.eai_exp[0], 1.15677655725858e08)
        self.assertAlmostEqual(imp.eai_exp[-1], 7.528669956120645e07)
        self.assertAlmostEqual(imp.tot_value, 6.570532945599105e11)
    ...
    def test_calc_impact_transf_pass(self):
        ...
        imp, risk_transf = entity.measures.get_measure(
            name="Beach nourishment", haz_type="TC"
        ).calc_impact(entity.exposures, entity.impact_funcs, hazard)

        self.assertAlmostEqual(imp.aai_agg, 6.280804242609713e09)
        self.assertAlmostEqual(imp.at_event[0], 0)
        self.assertAlmostEqual(imp.at_event[12], 8.648764833437817e07)
        self.assertAlmostEqual(imp.at_event[41], 500000000)
        self.assertAlmostEqual(imp.at_event[11890], 6.498096646836635e07)
        ...
        self.assertTrue(np.array_equal(imp.date, hazard.date))
        self.assertEqual(imp.event_name, hazard.event_name)
        self.assertEqual(risk_transf.aai_agg, 2.3139691495470852e08)
