...
class StormEurope(Hazard):
    ...
    def _read_one_nc(cls, file_name, centroids, intensity_thres):
        ...
        stacked = ncdf.max_wind_gust.stack(intensity=("latitude", "longitude", "time"))
    ...
    def from_cosmoe_file(
        ...
        haz = cls(
            ...
            event_name=[
                ...
                for date_i, ens_i in zip(date_to_str(date), stacked.epsd_1.values + 1)
            ],
            frequency=np.divide(np.ones_like(event_id), np.unique(ncdf.epsd_1).size),
        )
    ...
    def from_icon_grib(
        ...
        haz = cls(
            ...
            frequency=np.divide(np.ones_like(event_id), np.unique(stacked.number).size),
        )
        haz.check()
