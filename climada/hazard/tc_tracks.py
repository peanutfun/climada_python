...
class TCTracks:
    ...
    def from_ibtracs_netcdf(
        ...
        for tc_var in phys_vars:
            ...
            ag_vars = [
                ag_var for ag_var in ag_vars if ag_var in ibtracs_ds.data_vars.keys()
            ]
            if len(ag_vars) == 0:
        ...
        ibtracs_ds["id_no"] = (
            ibtracs_ds.sid.str.replace(b"N", b"0").str.replace(b"S", b"1").astype(float)
        )
        ...
        for i_track, t_msk in enumerate(ibtracs_ds.valid_t.data):
            ...
            with warnings.catch_warnings():
                ...
                track_ds["rmw"] = (
                    track_ds.rmw.ffill(dim="date_time", limit=1)
                    .bfill(dim="date_time", limit=1)
                    .fillna(0)
                )
                track_ds["roci"] = (
                    track_ds.roci.ffill(dim="date_time", limit=1)
                    .bfill(dim="date_time", limit=1)
                    .fillna(0)
                )
                track_ds["poci"] = track_ds.poci.ffill(dim="date_time", limit=4).bfill(
                    dim="date_time", limit=4
                )
    ...
    def generate_centroids(self, res_deg, buffer_deg):
        ...
        return Centroids(latitude=lat, longitude=lon)
    ...
    def write_hdf5(self, file_name, complevel=5):
        ...
        encoding = {
            v: dict(zlib=True, complevel=complevel) for v in ds_combined.data_vars
        }
    ...
    def _one_interp_data(track, time_step_h, land_geom=None):
        ...
        if track.time.size < 2:
            ...
            track_int.coords["lat"] = track.lat.resample(time=time_step).interpolate(
                method
            )
            track_int.attrs["category"] = set_category(
                track_int.max_sustained_wind.values, track_int.max_sustained_wind_unit
            )
...
def _raise_if_legacy_or_unknown_hdf5_format(file_name):
    ...
    try:
        ...
        with xr.open_dataset(file_name, group="track0") as ds_track:
            ...
            is_legacy = (
                "time" in ds_track.dims and "max_sustained_wind" in ds_track.variables
            )
    ...
    for ex, max_o in zip(explanatory, order):
        if isinstance(max_o, tuple):
            ...
            for i, max_o_i in enumerate(max_o):
                ...
                slope_1 = 1.0 / (max_o[i + 1] - max_o_i) if i + 1 < len(max_o) else 0
...
def ibtracs_add_official_variable(ibtracs_ds, tc_var, add_3h=False):
    ...
    agency_map = {
        a.encode("utf-8"): available_agencies.index(a) + 1
        if a in available_agencies
        else 0
    ...
    if add_3h:
        ...
        official_agency_ix_interp.values[
            official_agency_ix_interp.isnull().values
        ] = 0.0
