...
def download_icon_grib(
    ...
    LOGGER.info(
        "Downloading icon grib files of model %s for parameter %s with starting date %s.",
        model_name,
        parameter_name,
        ...
        if not bz2_pathfile_i.exists():
            try:
                ...
                if run_datetime > (dt.datetime.utcnow() - dt.timedelta(hours=6)):
                    err_msg += (
                        f"Forecast file {file_name_i} might not yet be available "
                        f"on {url}. Wait a few hours. "
                    )
                elif run_datetime < (dt.datetime.utcnow() - dt.timedelta(hours=24)):
                    err_msg += (
                        f"Forecast file {file_name_i} might no longer be available "
...
def delete_icon_grib(
    ...
    _, file_name, lead_times = _create_icon_grib_name(
        run_datetime, model_name, parameter_name, max_lead_time
    )
    download_path = (
        CONFIG.local_data.save_dir.dir() if download_dir is None else Path(download_dir)
    ...
    for lead_i in lead_times:
        ...
        if full_path_name_i.exists():
            ...
            LOGGER.warning(
                "File %s does not exist and could not be deleted.", full_path_name_i
            )
...
def _create_icon_grib_name(
    ...
    if (model_name == "icon-eu-eps") & (parameter_name == "vmax_10m"):
        ...
        lead_times = np.concatenate(
            (np.arange(1, 49), np.arange(51, 73, 3), np.arange(78, 121, 6))
        )
        ...
        file_extension = (
            "_storm_europe_icon_"  # this string completes the filename on the server
        )
        file_extension_2 = ""  # this string completes the filename on the server
        ...
        raise ValueError(
            f"Download for model {model_name} and parameter {parameter_name} "
    ...
    if not max_lead_time:
        ...
        LOGGER.warning(
            "Parameter max_lead_time %s is bigger than maximum "
            "available files. max_lead_time is adjusted to %s.",
            max_lead_time,
            max_lead_time_default,
        )
        max_lead_time = max_lead_time_default
...
def download_icon_centroids_file(model_name="icon-eu-eps", download_dir=None):
    ...
    if model_name == "icon-eu-eps":
        ...
        raise ValueError(
            f"Creation of centroids for the icon model {model_name} "
    ...
    if not nc_pathfile.exists():
        if not bz2_pathfile.exists():
            try:
                download_file(url + file_name, download_dir=download_path)
