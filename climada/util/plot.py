...
def _plot_scattered_data(
    ...
    **kwargs,
    ...
    if axes is None:
        ...
        _, axes, fontsize = make_map(
            num_im, proj=proj_plot, figsize=figsize, adapt_fontsize=adapt_fontsize
        )
    else:
        fontsize = None
    axes_iter = axes
    ...
    for array_im, axis, tit, name, coord in zip(
        list_arr, axes_iter.flatten(), list_tit, list_name, list_coord
    ):
        if coord.shape[0] != array_im.size:
            raise ValueError(
                f"Size mismatch in input array: {coord.shape[0]} != {array_im.size}."
            )

        # Binned image with coastlines
        if isinstance(proj, ccrs.PlateCarree):
            xmin, ymin, xmax, ymax = u_coord.latlon_bounds(
                coord[:, 0], coord[:, 1], buffer=buffer
            )
            extent = (xmin, xmax, ymin, ymax)
        else:
            extent = _get_borders(
                coord, buffer=buffer, proj_limits=proj.x_limits + proj.y_limits
            )
        ...
        if method == "hexbin":
            ...
            mappable = axis.hexbin(
                coord[:, 1], coord[:, 0], C=array_im, transform=proj, **kwargs
            )
        else:
...
def geo_im_from_array(
    ...
    **kwargs
    ...
    for array_im, axis, tit, name in zip(
        list_arr, axes_iter.flatten(), list_tit, list_name
    ):
        if coord.shape[0] != array_im.size:
            raise ValueError(
                f"Size mismatch in input array: {coord.shape[0]} != {array_im.size}."
        ...
        if smooth or not is_reg:
            ...
            grid_im = griddata((coord[:, 1], coord[:, 0]), array_im, (grid_x, grid_y))
        else:
        ...
        axis.set_extent(
            (extent[0] - mid_lon, extent[1] - mid_lon, extent[2], extent[3]), crs=proj
        )
        ...
        img = axis.pcolormesh(
            grid_x - mid_lon, grid_y, np.squeeze(grid_im), transform=proj, **kwargs
        )
...
def geo_scatter_categorical(
    array_sub, geo_coord, var_name, title, cat_name=None, adapt_fontsize=True, **kwargs
):
    """
    ...
    **kwargs
    ...
    array_sub_unique, array_sub_cat = np.unique(
        array_sub, return_inverse=True
    )  # flattens array
    ...
    if "cmap" in kwargs:
        ...
        if isinstance(cmap_arg, str):
            ...
            cmap = mpl.colors.ListedColormap(cmap_arg.colors[:array_sub_n])
        else:
            raise TypeError(
                "if cmap is given it must be either a str or a ListedColormap"
            )
    else:
        # default qualitative colormap
        cmap_name = CMAP_CAT
        cmap = mpl.colors.ListedColormap(cm.get_cmap(cmap_name).colors[:array_sub_n])

    if array_sub_n > cmap.N:
        LOGGER.warning(
            "More than %d categories cannot be plotted accurately "
...
def make_map(num_sub=1, figsize=(9, 13), proj=ccrs.PlateCarree(), adapt_fontsize=True):
    ...
    fig, axis_sub = plt.subplots(
        num_row, num_col, figsize=figsize, subplot_kw=dict(projection=proj)
    )
    axes_iter = axis_sub
...
def add_populated_places(axis, extent, proj=ccrs.PlateCarree(), fontsize=None):
    ...
    """
    shp_file = shapereader.natural_earth(
        resolution="50m", category="cultural", name="populated_places_simple"
    )

    shp = shapereader.Reader(shp_file)
    ext_pts = list(box(*u_coord.toggle_extent_bounds(extent)).exterior.coords)
    ext_trans = [
        ccrs.PlateCarree().transform_point(pts[0], pts[1], proj) for pts in ext_pts
    ]
        ...
        if ext_trans[2][0] < point.x <= ext_trans[0][0]:
            if ext_trans[0][1] < point.y <= ext_trans[1][1]:
                ...
                axis.text(
                    point.x,
                    point.y,
                    _ensure_utf8(rec.attributes["name"]),
